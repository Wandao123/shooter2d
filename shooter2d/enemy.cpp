...
public:
	SmallRedEnemy(const Vector2& position)
		: Enemy(position, std::make_unique<Sprite>("images/Enemy.png", Sprite::BlendMode::Blend), std::make_unique<CircleCollider>(Enemy::Width * 0.5f), EffectManager::EffectID::RedCircle)
	{
...
public:
	SmallBlueEnemy(const Vector2& position)
		: Enemy(position, std::make_unique<Sprite>("images/Enemy.png", Sprite::BlendMode::Blend), std::make_unique<CircleCollider>(Enemy::Width * 0.5f), EffectManager::EffectID::BlueCircle)
	{
		for (int j = 0; j < static_cast<int>(clips[0].size()); j++) {
